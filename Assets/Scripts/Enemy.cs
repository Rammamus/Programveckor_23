...
public class Enemy : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.CompareTag("Player"))
        {
            ...
            if(collider.GetComponent<kar�ktar>( ) != null && isDog == true)
            {
                collider.GetComponent<kar�ktar>().playHP -= dog.dmg;
            }
            if (collider.GetComponent<kar�ktar>() != null && isGlass == true)
            {
                collider.GetComponent<kar�ktar>().playHP -= glass.dmg;
            }
            if (collider.GetComponent<kar�ktar>() != null && isBabyGlass == true)
            {
                collider.GetComponent<kar�ktar>().playHP -= babyGlass.dmg;
            }
        }
